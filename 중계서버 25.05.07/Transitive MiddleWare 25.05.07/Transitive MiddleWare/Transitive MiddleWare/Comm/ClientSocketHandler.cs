...
namespace BPNS.TransitiveMiddleware
{
    ...
    //    RFID_DATA_PROCESS = 1000,
    //    RFID_POBDATA_PROCESS = 1010,
    //    POBDISPLAY_DATA_REQ = 2010,
    //    POBDISPLAY_SETTING_REQ = 2011,
    //    HEALTH_DATA_PROCESS = 3000,
    //    DEV_ERROR_PROCESS = 3010,
    //}
    ...
    {
        NO_ERROR = 0x0000,
        DUPLICATE_DATA = 0x1001,
        DATA_LENGTH_ERROR = 0x1002,
        CRC_ERROR = 0x1003,
        DATA_FORMAT_ERROR = 0x1004,
        COMMAND_CODE_ERROR = 0x1005,
        OPERATION_CODE_ERROR = 0x1006,
        DATABASE_CONNECT_ERROR = 0x2001,
        UNDEFINED_ERROR = 0x9999,
    }
    ...
    class ClientSocketHandler : IDisposable
    {
                ...
                lock (parsingBufferCO)
                {
                    ...
                    else
                    {
                        ...
                        if (_nPacketSize > 0)
                        {
                            ...
                            Program.AppLog.Write("Recv(_packetBuffer) :(" + sMWID + ")[" + EdASCII.GetString(_packetBuffer, clientSocketHandler.ReceiveBufferOffset, _packetBuffer.Length) + "]");
                            Program.AppLog.Write("Recv(ReceivceBuffer) :(" + sMWID + ")[" + EdASCII.GetString(ReceivceBuffer, clientSocketHandler.ReceiveBufferOffset, ReceivceBuffer.Length) + "]");
                        }
                        else
                        {
        ...
        public void ParsePacketCheck()
        {
            ...
            //24.02.15 로그제외
            Program.AppLog.Write("RecvData : " + strRecvData);

            int iEOF = strRecvData.IndexOf("<CMD:EOF>");

            string sTrRecvCmd = string.Empty;
            ...
            if (iEOF > 0)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    strCommandCode = ex.ToString();
                    Program.AppLog.Write("Recv 처리시 Exception : " + strCommandCode);
                }

                string sRtn = string.Empty;


                //if (strCommandCode == "118REQ")
                //{
                //    string sResultValue = "0";
                //    string sResultMsg = "";

                //    DateTime dteStart = DateTime.Now;
                //    Program.AppLog.Write("start time : " + strCommandCode + ":" + dteStart.ToString("yyyy-MM-dd hh:mm:ss.fff"));
                ...
                //    ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode.Replace("REQ", "").Replace("REP", ""));

                //    SendResponse_New(sRtn, strCommandCode);

                //    Program.AppLog.Write("[pams - request : " + strCommandCode + ":" + sRtn);




                //    Program.AppLog.Write("send : " + strCommandCode);
                //    Program.AppLog.Write("receive : " + sRtn.ToString());

                //    DateTime dteEnd = DateTime.Now;
                //    Program.AppLog.Write("end time : " + strCommandCode + ":" + dteEnd.ToString("yyyy-MM-dd hh:mm:ss.fff"));


                //    TimeSpan TS = dteEnd - dteStart;
                //    double diffSec = TS.TotalSeconds;
                //    Program.AppLog.Write("execute time : " + strCommandCode + ":" + diffSec + "s");

                //    Dictionary<string, object> LogData = new Dictionary<string, object>();
                //    LogData.Add("workdiv", strCommandCode);
                ...
                //}
                //else if (strCommandCode == "911REQ")

                //if (strCommandCode == "911REQ")
                //{
                //    string sResultValue = "0";
                //    string sResultMsg = "";
                ...
                #region 24.09.02 수정
                //if (strCommandCode == "110REQ" || strCommandCode == "111REQ" || strCommandCode == "112REQ" || strCommandCode == "113REQ"
                //                 || strCommandCode == "117REQ" || strCommandCode == "119REQ" || strCommandCode == "120REQ" || strCommandCode == "121REQ"
                //                 )
                //{
                ...
                //    //실제 실행시는 필요
                //    response = TCPClient.request(strRecvData, "192.168.200.21", 12012, false);
                //    sRtn = response.ToString();
                ...
                //{
                //    //24.02.15 로그제외
                //    //Program.AppLog.Write("ParsePacket_New 진입" + " : " + strCommandCode  + " : "+ strRecvData);
                //    ParsePacket_New(sTrRecvCmd);
                //}
                #endregion 24.09.02 수정
            ...
            //JObject response = TCPClient.request(strRecvData, Properties.Settings.Default.TO_CTRL_HOST, Properties.Settings.Default.TO_CTRL_PORT, false);
            ...
            //if (iEOF > 0) strCommandCode = result["WORKDIV"].ToString();
            //else strCommandCode = "체크안됨";
            ...
            //JObject response = TCPClient.request(strRecvData, "192.168.200.21", 12012, false);
            ...
            //string strCommandCode = result["WORKDIV"].ToString();
            ...
            try
            {
                ...
                switch (strCommandCode)
                {
                    ...
                    case "000REQ":
                        ...
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 발행 TAG_ID 요청" + " : " + sIP);
                        C000REQ(result);
                    ...
                    case "001REQ":
                        ...
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 태그발급 대상리스트 요청" + " : " + sIP);
                        C001REQ(result);
                    ...
                    case "002REQ":
                        ...
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 태그재발행 대상 등록" + " : " + sIP);
                        C002REQ(result);
                    ...
                    case "003REQ":
                        ...
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : PDA 태그 재발행 처리" + " : " + sIP);
                        C003REQ(result);
                    ...
                    //GATE_ID: 게이트 ID 
                    case "004REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : MW(시작/중지/재시작)" + " : " + sIP);
                        C004REQ(result);
                        break;

                    //C005REQ(PDA SW 요청)
                    case "005REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : PDA SW 요청" + " : " + sIP);
                        C005REQ(result);
                        break;

                    //C010REQ(PAMS 상태 체크)
                    case "010REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : PAMS 상태 체크" + " : " + sIP);
                        C010REQ(result);
                        break;
                    ...
                    case "020REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 연계상태 요청" + " : " + sIP);
                        C020REQ_NEW(result);
                    ...
                    case "100REQ":
                        ...
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 로그인" + " : " + sIP);
                        C100REQ(result);
                    ...
                    //서고 목록 요청
                    case "110REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 서고 목록 요청" + " : " + sIP);
                        C110REQ(result);
                    ...
                    //서가 목록 요청
                    case "111REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 서가 목록 요청" + " : " + sIP);
                        C111REQ(result);
                    ...
                    //연 목록요청
                    case "112REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 연 목록요청" + " : " + sIP);
                        C112REQ(result);
                        break;

                    //단 목록요청
                    case "113REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 연단 목록요청" + " : " + sIP);
                        C113REQ(result);
                        break;

                    //박스 목록요청
                    case "114REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 보존박스 목록 요청" + " : " + sIP);
                        C114REQ(result);
                        break;

                    //기록물 상세정보 요청 데이터 조회
                    case "115REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 기록물 상세 요청" + " : " + sIP);
                        C115REQ(result);
                        break;

                    //관리번호 또는 제목으로 기록물 목록 검색
                    case "116REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 관리번호 또는 제목으로 기록물 목록 검색" + " : " + sIP);
                        C116REQ(result);
                        break;

                    //C117REQ
                    case "117REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 존 관리" + " : " + sIP);
                        C117REQ(result);
                        break;

                    //C118REQ
                    case "118REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : GATE정보 관리" + " : " + sIP);
                        C118REQ(result);
                        break;

                    //C119REQ
                    case "119REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 장비정보 관리" + " : " + sIP);
                        C119REQ(result);
                        break;

                    //C120REQ
                    case "120REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 프로그램 관리" + " : " + sIP);
                        C120REQ(result);
                        break;
                    ...
                    //장비별프로그램관리
                    case "121REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 장비별프로그램 관리" + " : " + sIP);
                        C121REQ(result);
                        break;

                    //서가별 서고 현황 요청(서고현황)
                    case "200REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 서고현황 요청" + " : " + sIP);
                        C200REQ(result);
                        break;

                    //C201REQ
                    case "201REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 서고별현황 요청" + " : " + sIP);
                        C201REQ(result);
                        break;

                    //C202REQ
                    case "202REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 연별현황 요청" + " : " + sIP);
                        C202REQ(result);
                        break;

                    //C203REQ
                    case "203REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 단별 기록물 목록 요청" + " : " + sIP);
                        C203REQ(result);
                        break;
                    ...
                    //C400REQ(배치 : 배치의뢰서 요청
                    case "400REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 배치의뢰서 요청" + " : " + sIP);
                        C400REQ(result);
                        break;

                    //C401REQ(배치 : 배치대상물 목록)
                    case "401REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 배치대상물 목록 요청" + " : " + sIP);
                        C401REQ(result);
                        break;

                    //402
                    case "402REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 기록물 배치 요청" + " : " + sIP);
                        C402REQ(result);
                        break;

                    //403
                    case "403REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 기록물 배치" + " : " + sIP);
                        C403REQ(result);
                        break;
                    ...
                    //502
                    case "502REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 기록물 배치(재배치요청)" + " : " + sIP);
                        C502REQ(result);
                        break;
                    ...
                    //C600REQ
                    case "600REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 반출서 목록 요청" + " : " + sIP);
                        C600REQ(result);
                        break;

                    //C601REQ
                    case "601REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 반출 기록물 목록 요청" + " : " + sIP);
                        C601REQ(result);
                        break;

                    //C602REQ
                    case "602REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 반출 처리 요청" + " : " + sIP);
                        C602REQ(result);
                        break;

                    case "650REQ":
                        //C650REQ(무단 반출입 제외대상 등록 요청)
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 무단 반출/입 제외 대상 등록 요청" + " : " + sIP);
                        C650REQ(result);
                    ...
                    //C700REQ
                    case "700REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 반입서 목록 요청 요청" + " : " + sIP);
                        C700REQ(result);
                        break;
                    ...
                    //반입기록물 목록 요청
                    case "701REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 반입기록물 목록 요청" + " : " + sIP);
                        C701REQ(result);
                        break;
                    ...
                    //반입 처리 요청
                    case "702REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 반입 처리 요청" + " : " + sIP);
                        C702REQ(result);
                        break;

                    //C800REQ
                    case "800REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 미반입 목록 요청" + " : " + sIP);
                        C800REQ(result);
                        break;
                    ...
                    //미승인 무단 반출/입 목록 조회
                    case "801REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 미승인 무단 반출/입 목록 조회 요청" + " : " + sIP);
                        C801REQ(result);
                        break;
                    ...
                    case "802REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 미승인 무단 반출/입 목록 결과 전송 요청" + " : " + sIP);
                        C802REQ(result);
                        break;
                    ...
                    //정수점검 계획서 목록 조회
                    case "900":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 정수점검 계획서 목록 조회 요청" + " : " + sIP);
                        C900(result);
                        break;
                    ...
                    //정수점검 계획서 기록물 목록 조회
                    case "901REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 정수점검 계획서 기록물 목록 조회 요청" + " : " + sIP);
                        C901REQ(result);
                        break;
                    ...
                    //정수점검 결과 처리
                    case "902REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 정수점검 결과 처리 요청" + " : " + sIP);
                        C902REQ(result);
                        break;
                    ...
                    //작업내역 목록 요청
                    case "911REQ":
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("RCV DATA :" + strCommandCode + " : 작업내역 목록 요청" + " : " + sIP);
                        C911REQ(result);
                        break;
        ...
        private void SendErrResponse(string sWORKDIV, string sErrCD = "1", string sErrMsg = "Server 예외 에러 발생")
        {
        ...
        public void C000REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/inout/rfidTagReExecImp.do?WORKDIV=RfidTagReExecImp&";  //USER_ID=pams2&PWD=pams1!";
                    string sPara1 = request["EQ_ID"] == null ? "" : request["EQ_ID"].ToString();
                    string sPara2 = request["USER_ID"] == null ? "" : request["USER_ID"].ToString();
                    string sPara3 = string.Empty;

                    JArray list = (JArray)(request["rows"]);
                    for (int i = 0; i < list.Count; i++)
                    {
                        if (list[i]["FILE_BREAK_ID"] != null)
                        {
                            ((JArray)(request["rows"]))[i]["FILE_BREAK_ID"] = list[i]["FILE_BREAK_ID"].ToString().Trim().Replace("00", "");
                        }
                    }

                    sPara3 = Newtonsoft.Json.JsonConvert.SerializeObject(request["rows"], Newtonsoft.Json.Formatting.Indented);

                    string sPara = string.Format("EQ_ID={0}&USER_ID={1}&LIST={2}", sPara1, sPara2, sPara3);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "RfidTagReExecImp");
        ...
        public void C001REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/iss/rfidReTagBindListMsg.do?WORKDIV=RfidReTagBindListMsg";

                    string sPara = string.Empty;

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "RfidReTagBindListMsg");
        ...
        public void C002REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/iss/rfidAddReTagMsg.do?WORKDIV=RfidAddReTagMsg&";  //USER_ID=pams2&PWD=pams1!";
                    string sPara1 = request["PAMS_MGMT_NO"] == null ? "" : request["PAMS_MGMT_NO"].ToString();
                    string sPara2 = request["PRSDT_NM"] == null ? "" : request["PRSDT_NM"].ToString();
                    string sPara3 = request["PROD_ORG_NM"] == null ? "" : request["PROD_ORG_NM"].ToString();
                    string sPara4 = request["PROD_STRT_YY"] == null ? "" : request["PROD_STRT_YY"].ToString();
                    string sPara5 = request["PRSRV_PRD"] == null ? "" : request["PRSRV_PRD"].ToString();
                    string sPara6 = request["TAG_ID"] == null ? "" : request["TAG_ID"].ToString();
                    string sPara7 = request["ISSUE_TYPE"] == null ? "" : request["ISSUE_TYPE"].ToString();
                    string sPara8 = "";
                    try
                    {
                        sPara8 = request["FILE_BREAK_ID"] == null ? "" : request["FILE_BREAK_ID"].ToString().Trim().Replace("00", "");
                    }
                    catch (Exception e)
                    {
                        Program.SysLog.Write(this, "C002REQ FILE_BREAK_ID 파싱 Exception", e);
                    }

                    string sPara = string.Format("PAMS_MGMT_NO={0}&PRSDT_NM={1}&PROD_ORG_NM={2}&PROD_STRT_YY={3}&PRSRV_PRD={4}&TAG_ID={5}&ISSUE_TYPE={6}&FILE_BREAK_ID={7}", sPara1, sPara2, sPara3, sPara4, sPara5, sPara6, sPara7, sPara8);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "RfidAddReTagMsg");
        ...
        public void C003REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    #region DB로 처리하기 위한 로직
                    //C003REQ_SAVE(request);
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/iss/rfidTagReExecImp.do?WORKDIV=RfidTagReExecImp&";  //USER_ID=pams2&PWD=pams1!";
                    string sPara1 = request["EQ_ID"] == null ? "" : request["EQ_ID"].ToString();
                    string sPara2 = request["USER_ID"] == null ? "" : request["USER_ID"].ToString();
                    string sPara3 = string.Empty;
        ...
        public void C010REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/common/alivecheck.do?WORKDIV=ALIVECHECK";

                    string sPara = string.Empty;

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "ALIVECHECK");
        ...
        /// </summary>
        /// <param name="request"></param>
        public void C020REQ_NEW(JObject request)
        {
            string sWORKDIV = "020";
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    DataRow dr = dt.NewRow();
                    dr["connectionKind"] = "PAMS";
                    dr["connectionStatus"] = "T";
                    dr["lastConnectionTime"] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

                    DataRow dr2 = dt.NewRow();
                    dr2["connectionKind"] = "RFID_MW";
                    dr2["connectionStatus"] = "T";
                    dr2["lastConnectionTime"] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

                    DataRow dr3 = dt.NewRow();
                    dr3["connectionKind"] = "EMS";
                    dr3["connectionStatus"] = "F";
                    dr3["lastConnectionTime"] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
        ...
        public void C100REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/common/login.do?WORKDIV=LOGIN&";  //USER_ID=pams2&PWD=pams1!";
                    string sPara1 = request["USER_ID"] == null ? "" : request["USER_ID"].ToString();
                    string sPara2 = request["PWD"] == null ? "" : request["PWD"].ToString();
                    string sPara = string.Format("USER_ID={0}&PWD={1}", sPara1, sPara2);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "LOGIN");
        ...
        public void C114REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/boxListMsg.do?";
                    string sPara1 = request["BKROOM_ID"] == null ? "" : request["BKROOM_ID"].ToString();
                    string sPara2 = request["BKSTND_ID"] == null ? "" : request["BKSTND_ID"].ToString();
                    string sPara3 = request["COL_NO"] == null ? "" : request["COL_NO"].ToString();
                    string sPara4 = request["ROW_NO"] == null ? "" : request["ROW_NO"].ToString();

                    string sPara = string.Format("BKROOM_ID={0}&BKSTND_ID={1}&COL_NO={2}&ROW_NO={3}", sPara1, sPara2, sPara3, sPara4);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "114");
        ...
        public void C115REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    //string sCall = "http://172.30.234.6:9091/rfid/binddoc/bindViewInfoMsg.do?WORKDIV=BINDVIEW&";
                    //string sPara1 = request["PAMS_MGMT_NO"] == null ? "" : request["PAMS_MGMT_NO"].ToString();
                    //string sPara2 = request["BIND_TTL"] == null ? "" : request["BIND_TTL"].ToString();
                    //string sPara3 = request.ContainsKey("ADDINFO") == true ? (request["ADDINFO"] == null ? "" : request["ADDINFO"].ToString()) : "";
                    //string sPara = string.Format("PAMS_MGMT_NO={0}&BIND_TTL={1}&ADDINFO={2}", sPara1, sPara2, sPara3);


                    string sCall = "http://172.30.234.6:9091/rfid/binddoc/bindViewInfoMsg.do?WORKDIV=BINDVIEW&";
                    string sPara1 = request["PAMS_MGMT_NO"] == null ? "" : request["PAMS_MGMT_NO"].ToString();
                    string sPara2 = request["BIND_TTL"] == null ? "" : request["BIND_TTL"].ToString();
                    string sPara3 = request.ContainsKey("TAG_ID") == true ? (request["TAG_ID"] == null ? "" : request["TAG_ID"].ToString()) : "";
                    string sPara4 = request.ContainsKey("ADDINFO") == true ? (request["ADDINFO"] == null ? "" : request["ADDINFO"].ToString()) : "";
                    string sPara = string.Format("PAMS_MGMT_NO={0}&BIND_TTL={1}&TAG_ID={2}&ADDINFO={3}", sPara1, sPara2, sPara3, sPara4);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "BINDVIEW");
        ...
        public void C116REQ(JObject request)
        {
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    string sMgmtNo = request["PAMS_MGMT_NO"] == null ? "" : request["PAMS_MGMT_NO"].ToString();

                    if (sMgmtNo.Trim().Length > 0)
                    {
                        string sTagNo = string.Empty;
                        string sBindTtl = string.Empty;
                        DataTable dtTag = TagInfoSelect(sMgmtNo);

                        if (dtTag == null)
                        {

                        }
                        else if (dtTag.Rows.Count > 0)
                        {
                            sTagNo = dtTag.Rows[0]["TAG_ID"].ToString().Trim();
                        }


                            //response = PamsClient.connect("com.hs.pams.common.server.bkroom.BindSrchMsg", request);
                            string sCall = "http://172.30.234.6:9091/rfid/binddoc/bindSrchMsg.do?";  //USER_ID=pams2&PWD=pams1!";
                        string sPara1 = "";
                        string sPara2 = "";
                        string sPara3 = sTagNo;
                        string sPara = string.Format("PAMS_MGMT_NO={0}&BIND_TTL={1}&TAG_ID={2}", sPara1, sPara2, sPara3);

                        RestAPI RA = new RestAPI();
                        response = RA.CallAPI(sCall + sPara, "116");
            ...
            {
                //Program.SysLog.Write(this, "기록물 상세 요청 오류(" + sWORKDIV + ")" + ", RcvData:" + request.ToString(), e);
                Program.SysLog.Write(this, "기록물 상세 요청 오류(" + sWORKDIV + ")" + ", RcvData:" + request.ToString());
        ...
        #region 기록물 상세 요청(관리번호 또는 제목으로 기록물 목록 검색) - Ori : 속도 문제로 115로 처리 하도록 수정, 24.12.05
        ///// <summary>
        ///// 기록물 상세 요청(관리번호 또는 제목으로 기록물 목록 검색)
        ///// </summary>
        ///// <param name="request"></param>
        ...
        public DataTable TagInfoSelect(string sMgmtNo)
        {
            Hashtable htParamsHashtable;
            DataSet dsResult;
            ...
            htParamsHashtable.Add("IN_PAMS_MGMT_NO", sMgmtNo);

            dsResult = ((MainForm)(Application.OpenForms["MainForm"])).dbManagement.ExecuteSelectProcedure("PAMSDATA.TRFID_TAG_SELECT", htParamsHashtable, Properties.Settings.Default.DB_Timeout);
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/bkroomViewSttsMsg.do?WORKDIV=BKSTNDLIST&";
                    string sPara1 = request["BKROOM_ID"] == null ? "" : request["BKROOM_ID"].ToString();

                    string sPara = string.Format("BKROOM_ID={0}", sPara1);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "BKSTNDLIST");
	        ...
	        */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/bkroomViewSttsMsg.do?WORKDIV=BKCOLLIST&";
                    string sPara1 = request["BKROOM_ID"] == null ? "" : request["BKROOM_ID"].ToString();
                    string sPara2 = request["BKSTND_ID"] == null ? "" : request["BKSTND_ID"].ToString();

                    string sPara = string.Format("BKROOM_ID={0}&BKSTND_ID={1}", sPara1, sPara2);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "BKCOLLIST");
	 ...
	 *  ===============================================
	        */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/bkroomViewSttsMsg.do?WORKDIV=BKROWNOLIST&";
                    string sPara1 = request["BKROOM_ID"] == null ? "" : request["BKROOM_ID"].ToString();
                    string sPara2 = request["BKSTND_ID"] == null ? "" : request["BKSTND_ID"].ToString();
                    string sPara3 = request["COL_NO"] == null ? "" : request["COL_NO"].ToString();

                    string sPara = string.Format("BKROOM_ID={0}&BKSTND_ID={1}&COL_NO={2}", sPara1, sPara2, sPara3);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "BKROWNOLIST");
	        ...
	        */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/bkroomViewSttsMsg.do?WORKDIV=BKBINDLIST&";
                    string sPara1 = request["BKROOM_ID"] == null ? "" : request["BKROOM_ID"].ToString();
                    string sPara2 = request["BKSTND_ID"] == null ? "" : request["BKSTND_ID"].ToString();
                    string sPara3 = request["COL_NO"] == null ? "" : request["COL_NO"].ToString();
                    string sPara4 = request["ROW_NO"] == null ? "" : request["ROW_NO"].ToString();
                    string sPara5 = string.Empty;

                    if (request.ContainsKey("PRSRV_BOX_ID"))
                    {
                        sPara5 = request["PRSRV_BOX_ID"] == null ? "" : request["PRSRV_BOX_ID"].ToString();
                        string sPara = string.Format("BKROOM_ID={0}&BKSTND_ID={1}&COL_NO={2}&ROW_NO={3}&PRSRV_BOX_ID={4}", sPara1, sPara2, sPara3, sPara4, sPara5);

                        RestAPI RA = new RestAPI();
                        response = RA.CallAPI(sCall + sPara, "BKBINDLIST");
                    }
                    else
                    {
                        string sPara = string.Format("BKROOM_ID={0}&BKSTND_ID={1}&COL_NO={2}&ROW_NO={3}", sPara1, sPara2, sPara3, sPara4);
	        ...
	        */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/rfidBkroomArrnMsg.do?WORKDIV=ARRNAPPLLIST";


                    string sPara = string.Empty;

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "ARRNAPPLLIST");
	        ...
	        */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/rfidBkroomArrnMsg.do?WORKDIV=ARRNINFOLIST&";
                    string sPara1 = request["ARRN_APPL_ID"] == null ? "" : request["ARRN_APPL_ID"].ToString();


                    string sPara = string.Format("ARRN_APPL_ID={0}", sPara1);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "ARRNINFOLIST");
	        ...
	        */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/rfidBkroomArrnMsg.do?WORKDIV=ARRNFIX&";
                    string sPara1 = request["ARRN_APPL_ID"] == null ? "" : request["ARRN_APPL_ID"].ToString();
                    string sPara2 = string.Empty;

                    JArray jList = (JArray)request["rows"];
                    ...
                    if (jList.Count > 200)
                    {
                        while (jList.Count > 0)
                        {
                            try
                            {
                                int iCnt = jList.Count > 200 ? 200 : jList.Count;
                                JArray JPartList = new JArray();

                                for (int iRow = 0; iRow < iCnt; iRow++)
                                {
                                    JPartList.Add(jList[iRow]);
                                }
                                ...
                                sPara2 = Newtonsoft.Json.JsonConvert.SerializeObject(JPartList, Newtonsoft.Json.Formatting.Indented);
                                sPara2 = sPara2.Replace("\\u0000", "");

                                JPartList.RemoveAll();

                                string sPara = string.Format("ARRN_APPL_ID={0}&LIST={1}", sPara1, sPara2);

                                RestAPI RA = new RestAPI();
                                response = RA.CallAPI(sCall + sPara, "ARRNFIX");
                            ...
                            {
                                Program.SysLog.Write(this, "반출 처리 요청 요청 오류(" + sWORKDIV + ")" + ", RcvData:" + sPara2, ex1);
                    ...
                    else
                    {
                        try
                        {
                            sPara2 = Newtonsoft.Json.JsonConvert.SerializeObject(request["rows"], Newtonsoft.Json.Formatting.Indented);
                            sPara2 = sPara2.Replace("\\u0000", "");
                        ...
                        {
                            Program.SysLog.Write(this, "반출 처리 요청 요청 오류(" + sWORKDIV + ")" + ", RcvData:" + request.ToString(), ex2);
	        ...
	        */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/rfidBkroomArrnMsg.do?WORKDIV=BINDREARRN&";
                    string sPara1 = string.Empty;

                    sPara1 = Newtonsoft.Json.JsonConvert.SerializeObject(request["rows"], Newtonsoft.Json.Formatting.Indented);
                    sPara1 = sPara1.Replace("\\u0000", "");

                    string sPara = string.Format("LIST={0}", sPara1);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "BINDREARRN");
	        ...
	        */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/bkroom/rfidBkroomArrnMsg.do?WORKDIV=REARRNFIX&";
                    string sPara1 = request["ARRN_APPL_ID"] == null ? "" : request["ARRN_APPL_ID"].ToString();
                    string sPara2 = string.Empty;
                    ...
                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "REARRNFIX");
        ...
        public void C600REQ(JObject request)
        {
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    response = ((MainForm)(Application.OpenForms["MainForm"])).JoInList;

                    if (response == null)
                    {
                        Program.AppLog.Write("반출서 목록 요청 오류 null(" + sWORKDIV + ")" + ", RcvData:" + request.ToString());
                        SendErrResponse(sWORKDIV);
                        return;
                    }
        ...
        #region 600REQ(반출 : 반출의뢰서 목록 요청) - Ori
        ///// <summary>
        ///// //600REQ(반출 : 반출의뢰서 목록 요청)
        ///// </summary>
        ///// <param name="request"></param>
        ...
        //    }
        //} 
        #endregion 600REQ(반출 : 반출의뢰서 목록 요청) - Ori
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/inout/inoutViewSttsMsg.do?WORKDIV=INOUTBINDOUT&";
                    string sPara1 = request["OUT_ID"] == null ? "" : request["OUT_ID"].ToString();
                    string sPara2 = string.Empty;


                    string sPara = string.Format("OUT_ID={0}", sPara1);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "INOUTBINDOUT");
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/inout/inoutProcSttsMsg.do?WORKDIV=INOUTPROCOUT&";
                    string sPara1 = request["OUT_ID"] == null ? "" : request["OUT_ID"].ToString();
                    string sPara2 = request["OUT_USER_ID"] == null ? "" : request["OUT_USER_ID"].ToString();
                    string sPara3 = string.Empty;

                    JArray jList = (JArray)request["rows"];
                    string sCnt = jList.Count.ToString();
                    ...
                    if (jList.Count > 200)
                    {
                        while (jList.Count > 0)
                        {
                            try
                            {
                                ...
                                sPara3 = Newtonsoft.Json.JsonConvert.SerializeObject(JPartList, Newtonsoft.Json.Formatting.Indented);
                                sPara3 = sPara3.Replace("\\u0000", "");

                                JPartList.RemoveAll();

                                string sPara = string.Format("OUT_ID={0}&OUT_USER_ID={1}&LIST={2}", sPara1, sPara2, sPara3);

                                RestAPI RA = new RestAPI();
                                response = RA.CallAPI(sCall + sPara, "INOUTPROCOUT");
                                ...
                                if (jList.Count == 0)
                                {
                                    ...
                                    SendResponse_New(sRtn, sWORKDIV + "REQ");
                                    //WORK_LIST_SAVE("OUT", DateTime.Now.ToString("yyyyMMdd"), sPara1, "완료", sCnt, sCnt);
                                    //TRFID_OUTIN_STTS_Save(request, "OUT");

                                    JObject result = new JObject();
                                    result = JObject.Parse(sRtn);
                                    INOUTRST(result);
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/npermn/addNpermnOutMsg.do?WORKDIV=EXCTNPERMN&";
                    string sPara1 = request["USERID"] == null ? "" : request["USERID"].ToString();
                    string sPara2 = request["EXTSTRTDT"] == null ? "" : request["EXTSTRTDT"].ToString();
                    string sPara3 = request["EXTENDDT"] == null ? "" : request["EXTENDDT"].ToString();

                    string sPara4 = string.Empty;

                    sPara4 = Newtonsoft.Json.JsonConvert.SerializeObject(request["rows"], Newtonsoft.Json.Formatting.Indented);

                    string sPara = string.Format("USERID={0}&EXTSTRTDT={1}&EXTENDDT={2}&LIST={3}", sPara1, sPara2, sPara3, sPara4);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "EXCTNPERMN");
        ...
        public void C700REQ(JObject request)
        {
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    response = ((MainForm)(Application.OpenForms["MainForm"])).JoOutList;

                    if (response == null)
                    {
                        Program.AppLog.Write("반입서 목록 요청 오류 null(" + sWORKDIV + ")" + ", RcvData:" + request.ToString());
                        SendErrResponse(sWORKDIV);
                        return;
                    }
        ...
        #region 700REQ(반입 : 반입서 목록 요청) - Ori
        ///// <summary>
        ///// //700REQ(반입 : 반입서 목록 요청)
        ///// </summary>
        ///// <param name="request"></param>
        ...
        //    }
        //} 
        #endregion 700REQ(반입 : 반입서 목록 요청) - Ori
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/inout/inoutViewSttsMsg.do?WORKDIV=INOUTBINDIN&";
                    string sPara1 = request["OUT_ID"] == null ? "" : request["OUT_ID"].ToString();
                    ...
                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "INOUTBINDIN");
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/inout/inoutProcSttsMsg.do?WORKDIV=INOUTPROCIN&";
                    string sPara1 = request["OUT_ID"] == null ? "" : request["OUT_ID"].ToString();
                    string sPara2 = request["OUT_USER_ID"] == null ? "" : request["OUT_USER_ID"].ToString();
                    string sPara3 = string.Empty;
                    ...
                    if (jList.Count > 200)
                    {
                        while (jList.Count > 0)
                        {
                            try
                            {
                                ...
                                RestAPI RA = new RestAPI();
                                response = RA.CallAPI(sCall + sPara, "INOUTPROCIN");
                                ...
                                if (jList.Count == 0)
                                {
                                    ...
                                    SendResponse_New(sRtn, sWORKDIV + "REQ");
                                    //WORK_LIST_SAVE("IN", DateTime.Now.ToString("yyyyMMdd"), sPara1, "완료", sCnt, sCnt);
                                    //TRFID_OUTIN_STTS_Save(request, "ARR");
                                    JObject result = new JObject();
                                    result = JObject.Parse(sRtn);
                                    INOUTRST(result);
                                }
                            ...
                            catch (Exception ex1)
                            {
                                if (jList.Count == 0)
                                {
                                    Program.SysLog.Write(this, "반입 처리 요청 오류(" + sWORKDIV + ")" + ", RcvData:" + request.ToString(), ex1);
                    ...
                    else
                    {
                        ...
                        {
                            Program.SysLog.Write(this, "반입 처리 요청 오류(" + sWORKDIV + ")" + ", RcvData:" + request.ToString(), ex2);
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/inout/inoutViewSttsMsg.do?WORKDIV=INOUTNOTIN&";
                    string sPara1 = request["PAMS_MGMT_NO"] == null ? "" : request["PAMS_MGMT_NO"].ToString();
                    string sPara2 = request["BIND_TTL"] == null ? "" : request["BIND_TTL"].ToString();


                    string sPara = string.Format("PAMS_MGMT_NO={0}&BIND_TTL={1}", sPara1, sPara2);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "INOUTNOTIN");
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/npermn/nermnBindListMsg.do?";
                    string sPara1 = request["INOUT_FLAG"] == null ? "" : request["INOUT_FLAG"].ToString();



                    string sPara = string.Format("INOUT_FLAG={0}", sPara1);
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/npermn/npermnTrtmReceiveMsg.do?WORKDIV=NPMNTRTMRCV&";
                    string sPara1 = request["INOUT_FLAG"] == null ? "" : request["INOUT_FLAG"].ToString();
                    string sPara2 = string.Empty;

                    sPara2 = Newtonsoft.Json.JsonConvert.SerializeObject(request["rows"], Newtonsoft.Json.Formatting.Indented);

                    string sPara = string.Format("INOUT_FLAG={0}&LIST={1}", sPara1, sPara2);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "NPMNTRTMRCV");
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/prsv/rfidPrsvPlanMsg.do?WORKDIV=PRSVPLAN";
                    string sPara1 = string.Empty;



                    string sPara = string.Empty;//string.Format("INOUT_FLAG={0}", sPara1);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "PRSVPLAN");
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/prsv/rfidPrsvPlanMsg.do?WORKDIV=PRSVPLANBIND&";
                    string sPara1 = request["PLAN_ID"] == null ? "" : request["PLAN_ID"].ToString();



                    string sPara = string.Format("PLAN_ID={0}", sPara1);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "PRSVPLANBIND");
            ...
            */
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    string sCall = "http://172.30.234.6:9091/rfid/prsv/rfidPrsvPlanMsg.do?WORKDIV=PRSVPLANRST&";
                    string sPara1 = request["PLAN_ID"] == null ? "" : request["PLAN_ID"].ToString();
                    string sPara2 = request["TASK_NM"] == null ? "" : request["TASK_NM"].ToString();
                    string sPara3 = string.Empty;
                    ...
                    if (jList.Count > 200)
                    {
                        while (jList.Count > 0)
                        {
                            try
                            {
                                ...
                                string sPara = string.Format("PLAN_ID={0}&TASK_NM={1}&LIST={2}", sPara1, sPara2, sPara3);

                                RestAPI RA = new RestAPI();
                                response = RA.CallAPI(sCall + sPara, "PRSVPLANRST");
                    ...
                    else
                    {
                        ...
                        {
                            Program.SysLog.Write(this, "정수점검 결과 처리 오류(" + sWORKDIV + ")" + ", RcvData:" + request.ToString(), ex2);
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    else
                    {
                        ...
                        Program.AppLog.Write("공통 : PING 오류 : [APP_CODE:" + strAppCode + ", APP_MSG:" + strAppMsg + "]");
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("공통 : PING 오류 : [APP_CODE:" + strAppCode + ", APP_MSG:" + strAppMsg + "]");
                    }
        ...
        public void WORK_LIST_SAVE(string IN_WORK_GBN, string IN_WORK_DATE, string IN_WORK_ID, string IN_STATUS, string IN_BOX_CNT, string IN_C_CNT)
        {
            string sWORKDIV = "999";
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    htParamsHashtable = new Hashtable();
                    htParamsHashtable.Clear();
                    htParamsHashtable.Add("IN_WORK_GBN", IN_WORK_GBN);
                    htParamsHashtable.Add("IN_WORK_DATE", IN_WORK_DATE);
                    htParamsHashtable.Add("IN_WORK_ID", IN_WORK_ID);
                    htParamsHashtable.Add("IN_STATUS", IN_STATUS);
                    htParamsHashtable.Add("IN_BOX_CNT", IN_BOX_CNT);
                    htParamsHashtable.Add("IN_C_CNT", IN_C_CNT);

                    ((MainForm)(Application.OpenForms["MainForm"])).dbManagement.ExecuteProcedure("PAMSDATA.T_WORKLIST_INSERT", htParamsHashtable, Properties.Settings.Default.DB_Timeout);
                    strAppCode = htParamsHashtable["O_APP_CODE"].ToString();
                    strAppMsg = htParamsHashtable["O_APP_MSG"].ToString();
                }
            }
            ...
            {
                Program.SysLog.Write(this, "WORK_LIST_SAVE 오류(" + sWORKDIV + ")" + ", RcvData:" + IN_WORK_GBN + "," + IN_WORK_DATE + "," + IN_WORK_ID + "," + IN_STATUS + "," + IN_BOX_CNT + "," + IN_C_CNT, e);
                SendErrResponse(sWORKDIV);
            }

        }

        public string C118REQEXP()
        {
            string sTmpReq118 = "{\r\n  \"WORKDIV\": \"118REP\",\r\n  \"RESULT\": \"0\",\r\n  \"count\": \"37\",\r\n  \"rows\": [\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G09\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G09\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000008\",\r\n      \"OUT_ZN_ID\": \"ZN000006\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G11\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G11\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000011\",\r\n      \"OUT_ZN_ID\": \"ZN000008\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G05\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G05\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000006\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G04\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G04\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000006\",\r\n      \"OUT_ZN_ID\": \"ZN000005\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G01\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G01\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000005\",\r\n      \"OUT_ZN_ID\": \"ZN000009\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G06\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G06\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000006\",\r\n      \"OUT_ZN_ID\": \"ZN000010\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G08\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G08\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000006\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G07\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G07\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000006\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G02\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G02\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000005\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G13\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G13\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000010\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G03\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G03\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000005\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G10\",\r\n      \"UPD_DTTM\": \"20231107172162479\",\r\n      \"GATE_NM\": \"지상1층G10\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000011\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G12\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상1층G12\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000008\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF1G14\",\r\n      \"UPD_DTTM\": \"20231107172162479\",\r\n      \"GATE_NM\": \"지상1층G14\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000004\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF2G03\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상2층G03\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000012\",\r\n      \"OUT_ZN_ID\": \"ZN000011\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF2G04\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상2층G04\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000012\",\r\n      \"OUT_ZN_ID\": \"ZN000011\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF2G05\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상2층G05\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000012\",\r\n      \"OUT_ZN_ID\": \"ZN000005\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF2G06\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상2층G06\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000012\",\r\n      \"OUT_ZN_ID\": \"ZN000005\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF2G01\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상2층G01\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000012\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFF2G02\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지상2층G02\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000012\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB2G04\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하2층G04\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000001\",\r\n      \"OUT_ZN_ID\": \"ZN000011\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB2G05\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하2층G05\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000002\",\r\n      \"OUT_ZN_ID\": \"ZN000001\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB2G06\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하2층G06\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000003\",\r\n      \"OUT_ZN_ID\": \"ZN000004\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB2G01\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하2층G01\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000010\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB2G02\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하2층G02\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000011\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB2G03\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하2층G03\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000005\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G01\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G01\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000004\",\r\n      \"OUT_ZN_ID\": \"ZN000011\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G02\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G02\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000004\",\r\n      \"OUT_ZN_ID\": \"ZN000011\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G03\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G03\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000004\",\r\n      \"OUT_ZN_ID\": \"ZN000010\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G06\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G06\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000010\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G09\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G09\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000011\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G04\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G04\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000004\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"N\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G05\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G05\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000012\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G07\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G07\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000011\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G08\",\r\n      \"UPD_DTTM\": \"20231107172162478\",\r\n      \"GATE_NM\": \"지하1층G08\",\r\n      \"GATE_TYPE\": \"CHK\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"목지점\",\r\n      \"IN_ZN_ID\": \"ZN000011\",\r\n      \"OUT_ZN_ID\": \"ZN000099\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G10\",\r\n      \"UPD_DTTM\": \"20231107172162479\",\r\n      \"GATE_NM\": \"지하1층G10\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000011\",\r\n      \"OUT_ZN_ID\": \"ZN000005\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    },\r\n    {\r\n      \"USE_SENSOR\": \"Y\",\r\n      \"REG_DTTM\": \"20151029024609280\",\r\n      \"GATE_ID\": \"RFB1G11\",\r\n      \"UPD_DTTM\": \"20231107172162479\",\r\n      \"GATE_NM\": \"지하1층G11\",\r\n      \"GATE_TYPE\": \"NOR\",\r\n      \"REG_USER_ID\": \"test\",\r\n      \"GATE_TYPE_NM\": \"일반\",\r\n      \"IN_ZN_ID\": \"ZN000011\",\r\n      \"OUT_ZN_ID\": \"ZN000005\",\r\n      \"UPD_USER_ID\": \"DAEMON\",\r\n      \"STATUS\": \"NOR\"\r\n    }\r\n  ]\r\n}";

            var vTmpReq118 = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, object>>(sTmpReq118);

            JArray rows = (JArray)vTmpReq118["rows"];

            var list = new List<JObject>();
            foreach (JObject o in rows.Children<JObject>())
            {
                var item = new JObject();
                foreach (JProperty p in o.Properties())
                {
                    ...
                    item.Add(name, JToken.FromObject(value));
                }

                //RFB1G01
                if (item["GATE_ID"].ToString() == "RFB1G01")
                {
                    string sDate = "2023-11-30 10:34:01";
                    string sGATE_NM = item["GATE_NM"].ToString();
                    item.Add("INOUT_LIST", JToken.FromObject(sDate + "|" + sGATE_NM + "|" + "PED0000008" + "|" + "반출"));
                    item["STATUS"] = "OUT";
                }
                else
                {
                    item.Add("INOUT_LIST", JToken.FromObject(" "));
                }

                //if (list.Count == 10)
                //{
                //    string sDate = "2023-11-07 09:30:31";
                //    string sGATE_NM = item["GATE_NM"].ToString();
                //    item.Add("INOUT_LIST", JToken.FromObject(sDate + "|" + sGATE_NM + "|" + "PED0000010" + "|" + "반입"));
                //    item["STATUS"] = "IN";
                //}
                //else if (list.Count == 20)
                //{
                //    string sDate = "2023-11-07 09:32:29";
                //    string sGATE_NM = item["GATE_NM"].ToString();
                //    item.Add("INOUT_LIST", JToken.FromObject(sDate + "|" + sGATE_NM + "|" + "PED0000010" + "|" + "반출"));
                //    item["STATUS"] = "OUT";
                //}
                //else
                //{
            ...
            Dictionary<string, object> aData = new Dictionary<string, object>();
            aData.Add("WORKDIV", "118REP");
            aData.Add("RESULT", vTmpReq118["count"]);
            aData.Add("ERROR", "");
            aData.Add("rows", list);
        ...
        /// </summary>
        /// <param name="joCmd"></param>
        public void C911REQ(JObject request)
        {
            string sWORKDIV = "911";
            JObject response = null;
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    string sCall = "http://172.30.234.6:9091/rfid/common/workList.do?WORKDIV=WORKLIST&";

                    string sGBN = request["GBN"] == null ? "" : request["GBN"].ToString();
                    string sSDATE = request["SDATE"] == null ? "" : request["SDATE"].ToString();
                    string sEDATE = request["EDATE"] == null ? "" : request["EDATE"].ToString();



                    string sPara = string.Format("GBN={0}&SDATE={1}&EDATE={2}", sGBN, sSDATE, sEDATE);

                    RestAPI RA = new RestAPI();
                    response = RA.CallAPI(sCall + sPara, "WORKLIST");
            ...
            {
                Program.SysLog.Write(this, "작업내역 요청 오류" + ", RcvData:" + request.ToString(), e);
                SendErrResponse(sWORKDIV);
            }
            ...
            //        htParamsHashtable = new Hashtable();
            //        htParamsHashtable.Clear();
            //        htParamsHashtable.Add("IN_GBN", sGBN);
            //        htParamsHashtable.Add("IN_SDATE", sSDATE);
            //        htParamsHashtable.Add("IN_EDATE", sEDATE);

            //        dsResult = ((MainForm)(Application.OpenForms["MainForm"])).dbManagement.ExecuteSelectProcedure("PAMSDATA.C911_SELECT", htParamsHashtable, Properties.Settings.Default.DB_Timeout);
            ...
            //            SendResponse_New(sRtn, "C911REQ");
            //        }
            //        else
            //        {
            //            ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("작업내역 조회 안됨.(" + request.ToString() + ")");
            //            Program.AppLog.Write("작업내역 조회 안됨.(" + request.ToString() + ")");
        ...
        /// <summary>
        /// //C002REQ(태그발급 리스트에 추가)
        /// </summary>
        /// <param name="joCmd"></param>
        public void C002REQ_Insert(JObject joCmd)
        {
            Hashtable htParamsHashtable;
            ...
            string sPAMS_MGMT_NO = joCmd["PAMS_MGMT_NO"] == null ? "" : joCmd["PAMS_MGMT_NO"].ToString();
            string sPRSDT_NM = joCmd["PRSDT_NM"] == null ? "" : joCmd["PRSDT_NM"].ToString();
            string sPROD_ORG_NM = joCmd["PROD_ORG_NM"] == null ? "" : joCmd["PROD_ORG_NM"].ToString();
            string sPROD_STRT_YY = joCmd["PROD_STRT_YY"] == null ? "" : joCmd["PROD_STRT_YY"].ToString();
            string sPRSRV_PRD = joCmd["PRSRV_PRD"] == null ? "" : joCmd["PRSRV_PRD"].ToString();
            string sTAG_ID = joCmd["TAG_ID"] == null ? "" : joCmd["TAG_ID"].ToString();
            string sISSUE_TYPE = joCmd["ISSUE_TYPE"] == null ? "" : joCmd["ISSUE_TYPE"].ToString(); //발행구분 : 재발행
            ...
            try
            {
                lock (oClientSocketCO)
                {
                    ...
                    ((MainForm)(Application.OpenForms["MainForm"])).dbManagement.ExecuteProcedure("PAMSDATA.C002REQ", htParamsHashtable, Properties.Settings.Default.DB_Timeout);
                    strAppCode = htParamsHashtable["O_APP_CODE"].ToString();
                    strAppMsg = htParamsHashtable["O_APP_MSG"].ToString();
                    ...
                    {
                        Program.AppLog.Write("PAMSDATA.C002REQ 오류 : [APP_CODE:" + strAppCode + ", APP_MSG:" + strAppMsg + "]" + sPAMS_MGMT_NO + sPRSDT_NM + sPROD_ORG_NM + sPROD_STRT_YY + sPRSRV_PRD + sTAG_ID + sISSUE_TYPE);
                        ((MainForm)(Application.OpenForms["MainForm"])).AddLogList("PAMSDATA.C002REQ 오류 : [APP_CODE:" + strAppCode + ", APP_MSG:" + strAppMsg + "]");
                    }
                }
            }
            catch (Exception e)
            {
                Program.SysLog.Write(this, "(PAMSDATA.C002REQ)태그발급 리스트에 추가 오류" + ", RcvData:" + joCmd.ToString(), e);
            }
        }
        ...
        public void TRFID_OUTIN_STTS_Save(JObject request, string sINOUT_FLAG)
        {
            ...
            string sPara1 = request["OUT_ID"] == null ? "" : request["OUT_ID"].ToString();
            string sPara2 = request["OUT_USER_ID"] == null ? "" : request["OUT_USER_ID"].ToString();
            string sEXTDT = DateTime.Now.ToString("yyyyMMdd");
            string sEXTENDDT = DateTime.Now.ToString("yyyyMMdd");

            JArray jList = (JArray)request["rows"];
            string sCnt = jList.Count.ToString();

            string sOUT_ID = string.Empty;
            string sOUT_SEQ = string.Empty;
            string sTAG_ID = string.Empty;
            string sPAMS_MGMT_NO = string.Empty;

            for (int iRow = 0; iRow < jList.Count; iRow++)
            {
                try
                {
                    sOUT_ID = jList[iRow]["OUT_ID"] == null ? "" : jList[iRow]["OUT_ID"].ToString();
                    sOUT_SEQ = jList[iRow]["OUT_SEQ"] == null ? "" : jList[iRow]["OUT_SEQ"].ToString();
                    sTAG_ID = jList[iRow]["TAG_ID"] == null ? "" : jList[iRow]["TAG_ID"].ToString();
                    sPAMS_MGMT_NO = jList[iRow]["PAMS_MGMT_NO"] == null ? "" : jList[iRow]["PAMS_MGMT_NO"].ToString();
                    ...
                    /*
                    IN_LOG_DTTM         IN  VARCHAR2,
                    IN_MGMT_NO          IN  VARCHAR2,
                    IN_TAG_ID           IN  VARCHAR2,
                    IN_GATE_ID          IN  VARCHAR2,
                    IN_PROC_CODE        IN  VARCHAR2,
                    IN_PROC_VAL         IN  VARCHAR2,
                    IN_INOUT_FLAG       IN  VARCHAR2,
                    IN_SDATE            IN  VARCHAR2,
                    IN_EDATE            IN  VARCHAR2, 
                    */
                    ...
                    string sProcVal = string.Format("{0}/{1}/{2}", sINOUT_FLAG, sEXTDT, sEXTENDDT);

                    htParamsHashtable.Add("IN_PROC_VAL", sProcVal);
                    htParamsHashtable.Add("IN_INOUT_FLAG", " ");
                    htParamsHashtable.Add("IN_SDATE", " ");
                    htParamsHashtable.Add("IN_EDATE", " ");
                ...
                {
                    Program.SysLog.Write(this, "TRFID_OUTIN_STTS SAVE 오류" + ", RcvData:" + sOUT_ID + "," + sOUT_SEQ + "," + sTAG_ID + "," + sPAMS_MGMT_NO + "," + sINOUT_FLAG, ex);
                }
            }
        ...
        public void C003REQ_SAVE(JObject request)
        {
            Hashtable htParamsHashtable = new Hashtable();
            ...
            for (int iRow = 0; iRow < jList.Count; iRow++)
            {
                try
                {
                    ...
                    htParamsHashtable.Clear();
                    htParamsHashtable.Add("IN_EQ_ID", sPara1);
                    htParamsHashtable.Add("IN_USER_ID", sPara2);
                    htParamsHashtable.Add("IN_PAMS_MGMT_NO", sPAMS_MGMT_NO);

                    ((MainForm)(Application.OpenForms["MainForm"])).dbManagement.ExecuteProcedure("PAMSDATA.C003REQ", htParamsHashtable, Properties.Settings.Default.DB_Timeout);
                    strAppCode = htParamsHashtable["O_APP_CODE"].ToString();
                    strAppMsg = htParamsHashtable["O_APP_MSG"].ToString();
                }
                catch (Exception ex)
                {
                    Program.SysLog.Write(this, "TRFID_OUTIN_STTS SAVE 오류" + ", RcvData:" + sPara1 + "," + sPara2 + "," + sPAMS_MGMT_NO, ex);
                }
            }
