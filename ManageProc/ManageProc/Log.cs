    ...
    public class Log
    {
        ...
        public void Write(string logString)
        {
            ...
            lock (oCriticalObject)
            {
                ...
                {
                    ((frmManageProc)(Application.OpenForms["frmManageProc"])).AddLogList("Log 파일 저장 오류[" + e.Message + "]");
                    //((ListBox)(Application.OpenForms["HHI_CNC_MW_MAIN"].Controls["logListBox"])).Items.Add("Log 파일 저장 오류[" + e.Message + "]");
                    //((RFID_MW)(Application.OpenForms["RFID_MW"])).AddLogList("Log 파일 저장 오류[" + e.Message + "]");
                }
                finally
                {
        ...
        public void Write(object sender, string logString)
        {
            ...
            lock (oCriticalObject)
            {
                ...
                {
                    ((frmManageProc)(Application.OpenForms["frmManageProc"])).AddLogList("Log 파일 저장 오류[" + e.Message + "]");
                    //((ListBox)(Application.OpenForms["RFID_MW"].Controls["logListBox"])).Items.Add("Log 파일 저장 오류[" + e.Message + "]");
                    //((RFID_MW)(Application.OpenForms["RFID_MW"])).AddLogList("Log 파일 저장 오류[" + e.Message + "]");
                }
                finally
                {
        ...
        public void Clearance()
        {
            ...
            {
                ((frmManageProc)(Application.OpenForms["frmManageProc"])).AddLogList("Log 파일 저장 오류[" + e.Message + "]");
                //((ListBox)(Application.OpenForms["RFID_MW"].Controls["logListBox"])).Items.Add("Log 파일 정리 오류[" + e.Message + "]");
                //((RFID_MW)(Application.OpenForms["RFID_MW"])).AddLogList("Log 파일 정리 오류[" + e.Message + "]");
        ...
        public void Clearance2()
        {
            ...
            try
            {
                ...
                lock (oCriticalObject)
                {
                    ...
                    strLogDir = Path.Combine(Application.StartupPath, string.Format("{0}", "Log"));

                    strStartDateFileName = (DateTime.Now - tsStorageTimeSpan).ToString("yyyyMMdd");
                    //diDirectoryInfo = new DirectoryInfo(strLogDir);

                    string[] Directories = Directory.GetDirectories(strLogDir);   // Defalut Folder
                    {
                        ...
                        foreach (string nodeDir in Directories)   // Folder list Search
                        {
                            if (isFiles(nodeDir))
                            {
                                diDirectoryInfo = new DirectoryInfo(nodeDir);
                                FileInfo[] ArrFileInfo = diDirectoryInfo.GetFiles("*.txt");

                                foreach (FileInfo fi in ArrFileInfo)
                                {
                                    if (strStartDateFileName.CompareTo(fi.Name.Replace("-", "")) > 0)
                                    {
                                        // 파일 삭제
                                        fi.Delete();
                                    }
                                ...
                                //File diDirectoryInfo.GetFiles(".txt");
                            }
                        }
                    }
            ...
            {
                ((frmManageProc)(Application.OpenForms["frmManageProc"])).AddLogList("Machine Log 파일 저장 오류[" + e.Message + "]");
                //((ListBox)(Application.OpenForms["RFID_MW"].Controls["logListBox"])).Items.Add("Machine Log 파일 정리 오류[" + e.Message + "]");
                //((RFID_MW)(Application.OpenForms["RFID_MW"])).AddLogList("Log 파일 정리 오류[" + e.Message + "]");
