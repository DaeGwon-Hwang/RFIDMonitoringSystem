...
namespace RFID_CONTROLLER.Controls.Editors {
	public partial class TextEditor : UserControl, IBaseEditor {
		...
		{
			get => _replaceText; set => _replaceText = value;
		}
		...
		public Data<string> GetData() {
			...
			if (!_replaceText.Equals(""))
				result[FieldName] = Editor.Text.Replace(_replaceText, "");
			else
		...
		public string GetValue() {
			if (!_replaceText.Equals(""))
				return Editor.Text.Replace(_replaceText, "");
			else
				return Editor.Text;
		}
		...
		public void Clear(bool fireEvent = true) {
			if(!DefaultCode.Equals(""))
				SetValue(DefaultCode, fireEvent);
			else
				SetValue("", fireEvent);
		}
		...
		private void Editor_MouseUp(object sender, MouseEventArgs e) {
			if(!_selectAllDone && Editor.SelectionLength == 0) {
				_selectAllDone = true;
				Editor.SelectAll();
			}
		}
