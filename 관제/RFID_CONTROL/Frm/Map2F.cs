...
{
    public partial class Map2F : Form
    {
        ...
        private sttPanelCnt F2SttPanelBig = new sttPanelCnt(6);
        public Map2F()
        {
            InitializeComponent();
        ...
        private void Map2F_Load(object sender, EventArgs e)
        {
            //게이트 아이콘 표시
            ...
            icons.Add("RFF2G01", new GateInfo(
                pictureBox1,
                FitWidthGap(Convert.ToInt32(pictureBox1.Size.Width * 9.5 / 100)),
            ...
            icons.Add("RFF2G02", new GateInfo(
                pictureBox1,
                FitWidthGap(Convert.ToInt32(pictureBox1.Size.Width * 29.2 / 100)),
            ...
            icons.Add("RFF2G03", new GateInfo(
                pictureBox1,
                FitWidthGap(Convert.ToInt32(pictureBox1.Size.Width * 33 / 100)),
            ...
            icons.Add("RFF2G04", new GateInfo(
                pictureBox1,
                FitWidthGap(Convert.ToInt32(pictureBox1.Size.Width * 35.5 / 100)),
                Convert.ToInt32(pictureBox1.Size.Height * 23 / 100)
                )
            );

            icons.Add("RFF2G05", new GateInfo(
                pictureBox1,
                FitWidthGap(Convert.ToInt32(pictureBox1.Size.Width * 81.5 / 100)),
        ...
        private void display()
        {
            ...
            if (rows != null)
            {
                for (int i = 0; i < rows.Count; i++)
                {
                    ...
                    if (icons.ContainsKey(key))
                    {
                        ...
                        if (gate != null)
                        {
                            ...
                            {
                                case "RFF2":
                                    if (row["STATUS"].ToString().Equals("NOR")) F2SttPanelBig.addNorCnt();
                                    else if (row["STATUS"].ToString().Equals("ERR")) F2SttPanelBig.addErrCnt();
                                    else if (row["STATUS"].ToString().Equals("IN")) F2SttPanelBig.addInCnt();
                                    else if (row["STATUS"].ToString().Equals("OUT")) F2SttPanelBig.addOutCnt();
                                    else F2SttPanelBig.addErrCnt();
                                    break;                               
                            }
                        }
                    }
                ...
                F2SttPanelBig.cntCheck();

                statusPanel1.norSttLabel = F2SttPanelBig.getNorCnt().ToString();
                statusPanel1.errSttLabel = F2SttPanelBig.getErrCnt().ToString();
                statusPanel1.inSttLabel = F2SttPanelBig.getInCnt().ToString();
                statusPanel1.outSttLabel = F2SttPanelBig.getOutCnt().ToString();

                F2SttPanelBig.resetAllCnt();
            }
        ...
        private void Map2F_FormClosing(object sender, FormClosingEventArgs e)
        {
            thread.Abort();
            this.Dispose();
        }
